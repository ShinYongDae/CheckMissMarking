...
BOOL CVision::Judge(stPtMtRst &stRst)
{
	...
	//if (((CCheckMissMarkingDlg*)m_pParent)->m_bUseCamMstModel)
	//{
...
void CVision::LoadCamMstModelBuf()
{
	...
	//UCHAR *pCamMstModelImg = ((CCheckMissMarkingDlg*)m_pParent)->m_pCamMstModelImg;
	//TiffData tdat;
...
BOOL CVision::Blob()
{
	...
	double dCameraRes = ((CCheckMissMarkingDlg*)m_pParent)->m_dPixelRes;
...
void CVision::DoHistogram()
{
	...
	MimHistogram(m_pMilBufModel->m_MilImage, HistResult);
	...
	for (i = 0; i < HIST_NUM_INTENSITIES; i++)
	{
		...
		m_nHistoRst[i] = YEnd[i] = (HistValues[i] / HIST_SCALE_FACTOR);
	}
